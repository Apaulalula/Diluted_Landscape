...
for(p in c(0.01,0.25,0.5)){
  for(f in c(0.1,0.3,0.5,0.7)){    
    ...
    result_hosts <-  Sensitivity_land %>% 
      filter(p_value == p) %>% 
      filter(forest_cover == f) %>% 
      filter(land == l) %>% 
      filter(guild== "resources")
    
    result_parasites <-Sensitivity_land %>% 
      filter(p_value == p) %>% 
      filter(forest_cover == f) %>% 
      filter(land == l) %>% 
    ...
    var_name <- paste0("l",l,"p", p,"TraitMatch_", f)
...
trait_match_land = data.frame(NULL)
...
for(p in c(0.01,0.25,0.5)){
  for(f in c(0.1,0.3,0.5,0.7)){
    ...
    TraitMatch <- get(paste0("l",l,"p", p,"TraitMatch_", f))[["matching_species"]] %>%
      filter(match != is.na(match)) %>% 
      mutate(p_value = p,
             forest_cover = f, 
             land = l)
    
    trait_match_land = rbind(trait_match_land, TraitMatch)
...
trait_match_land_mean = data.frame(NULL)
...
for(p in c(0.01,0.25,0.5)){
  for(f in c(0.1,0.3,0.5,0.7)){
    ...
    trait_match_land_mean = rbind(trait_match_land_mean, TraitMatch)
    }
  }
}
...
write.csv(trait_match_land_mean, "./Results_land_new/trait_match_land_mean.csv")
write.csv(trait_match_land, "./Results_land_new/trait_match_land.csv")
...
m_p_traitmatch_land <- trait_match_land_mean %>% 
...
# save data
write.csv(m_p_traitmatch_land, "./Results_land_new/m_p_traitmatch_land.csv")
