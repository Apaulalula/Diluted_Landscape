...
######## Load Data ######## 
outcomes_land <- read.csv("./Results_land_new/outcomes_land.csv") %>% column_to_rownames(var = "X")

# Work on data set ################################### 

# Verify unique levels of categorical variables
outcomes_land$parasite_species <- factor(outcomes_land$parasite_species)
outcomes_land$Landscape <- factor(outcomes_land$Landscape)
...
Ex_plot_data <- outcomes_land %>%  
  group_by(land, forest_cover, p_value) %>%
  mutate(land = case_when(land == "new" ~ "New landscape configuration",
                            land == "initial" ~ "Initial landscape configuration"
                            )) %>% 
  dplyr::summarise(
...
png(filename="./Results_new/ext_md_land.png", width=18, height=8, units="cm", res=600)
plot(Ex_plot)
dev.off()


# Network structure plots ####
df_plot <- outcomes_land_pca %>%
  mutate(
...
df_plot <- outcomes_land_pca %>%
  ...
  group_by(forest_cover, Land_use, land) %>%
  dplyr::summarise(
    mean_value = mean(PCA_axis1),
    se_value = sd(PCA_axis1) / sqrt(n()), .groups = "drop"
  ) %>% 
...
png(filename="./Results_new/PCA_ns_land.png", width=25, height=25, units="cm", res=600)
plot(final_plot)
dev.off()
...
png(filename="./Results_new/DG_plot1.png", width=15, height=7, units="cm", res=600)
plot(dg_plot1)
dev.off()

### Plot relation between relative degree, land use and host abundance  ####
dg_plot <-outcomes_land %>% 
  select(-p_value) %>% 
  mutate(land = case_when(land == "new" ~ "New landscape configuration",
                          land == "initial" ~ "Initial landscape configuration"
  )) %>% 
...
png(filename="./Results_new/PCA_rd_land.png", width=15, height=15, units="cm", res=600)
plot(final_plot)
dev.off()

## Trait matching plots ####
### Plot relation between trait matching, forest cover and network structure ####
TM_plot1 <-outcomes_land %>% 
  select(-p_value) %>% 
  mutate(land = case_when(land == "new" ~ "New landscape configuration",
                          land == "initial" ~ "Initial landscape configuration"
  )) %>% 
...
png(filename="./Results_new/tm_plot1.png", width=15, height=15, units="cm", res=600)
plot(TM_plot1)
dev.off()

### Plot relation between trait matching, habitat type and host abundance ####
TM_plot2 <-outcomes_land %>% 
  select(-p_value) %>% 
  mutate(land = case_when(land == "new" ~ "New landscape configuration",
                          land == "initial" ~ "Initial landscape configuration"
  )) %>% 
...
png(filename="./Results_new/PCA_tm_land.png", width=15, height=15, units="cm", res=600)
plot(final_plot)
dev.off()
