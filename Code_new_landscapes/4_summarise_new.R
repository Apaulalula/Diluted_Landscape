...
m_p_traitmatch_land <- read.csv("./Results_land_new/m_p_traitmatch_alpha.csv") %>% column_to_rownames(var = "X")
...
# gather model data
load("./Results_land_new/Sensitivity_land.RData")


# Calculate parasite global extinction

Ext_new <-  Sensitivity_land %>%
  filter(guild == "consumers") %>% 
  inner_join(prst_df) %>% 
  select(forest_cover,p_value, land, Name, species) %>% 
  mutate(Name = as.factor(Name)) %>%  
  group_by(forest_cover, p_value, land) %>%
...
res_h_land <- land_pv %>%
  select(forest_cover, p_value, frag_value, land, patch_no, Land_use, core_patch_count, edge_patch_count, disturbed_patch_count) %>% 
  full_join(Sensitivity_land) %>%
  filter(guild == "resources") %>% 
  mutate(species = as.character(species)) %>% 
  inner_join(host_df) %>% 
  select(forest_cover, p_value, frag_value, land, Land_use, core_patch_count, edge_patch_count, disturbed_patch_count, Name, z, pf) %>% 
  dplyr::mutate(Name = as.factor(Name)) %>%  
  group_by(forest_cover, p_value, frag_value, land, Land_use, core_patch_count, edge_patch_count, disturbed_patch_count, Name, pf) %>%
  dplyr::summarise(n= n()) %>%
...
h_abd_land <- land_pv %>% 
  ...
  right_join(interaction_new_df) %>% 
  unite("Interaction", parasite_species, host_species, sep= "/.") %>% 
  group_by(forest_cover,p_value,frag_value, land, Land_use, core_patch_count, edge_patch_count,  disturbed_patch_count, Interaction, value) %>% 
  ...
  select(forest_cover,p_value,frag_value,land, Land_use, parasite_species, Abd_type) %>% 
  group_by(forest_cover,p_value,frag_value, land, Land_use, parasite_species) %>% 
...
nh_df_a <- land_pv %>% 
  ...
  mutate(patch_id = as.character(patch_id)) %>% 
  right_join(interaction_new_df) %>%
  select(forest_cover,p_value, frag_value, land, Land_use, patch_id, parasite_species) %>% 
  group_by(forest_cover,p_value, frag_value, land, Land_use, patch_id, parasite_species) %>%
  dplyr::summarise(Count = n(), .groups = "drop") %>%
  inner_join(prst_net) %>% 
  mutate(Degree_rel = (Count/Degree)) %>% 
  group_by(forest_cover,p_value, frag_value, land, Land_use, parasite_species, ZoonoticStatus, ParGroup) %>%
...
outcomes_land <- nh_df_a %>%
  select(- Mean_count) %>%
  inner_join(h_abd_land) %>%
  mutate_if(is.character, as.factor) %>% 
  inner_join(m_p_traitmatch_land) %>% 
  select(-c(A_mtheta, A_nopartners, Count)) %>% 
  inner_join(Ext_new)

# Analyse outcomes distribution

outcomes_land$Mean_rel_sc <- as.numeric(scale(outcomes_land$Mean_rel))
outcomes_land$Amatch_sc <- as.numeric(scale(outcomes_land$A_match))
outcomes_land$Host_abd_sc <- as.numeric(scale(outcomes_land$Host_abd))
outcomes_land$Ext_rate_sc <- as.numeric(scale(outcomes_land$Ext_rate))

hist(outcomes_land$Mean_rel_sc)
hist(outcomes_land$Amatch_sc)
hist(outcomes_land$Host_abd_sc)
hist(outcomes_land$Ext_rate_sc)

# PCA axis 
pca_data <- new_net_mean %>% 
...
pca_data$PCA_axis1 <- pca_result$x[, 1] #connectance positive, size and modularity negative

outcomes_land_pca<- full_join(outcomes_land, pca_data)


# save data
write.csv(outcomes_land, "./Results_land_new/outcomes_land.csv")
