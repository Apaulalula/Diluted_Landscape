...
########## Load data ###############
land_ict<- read.csv("./Data/land_ict.csv") %>% column_to_rownames(var = "X.1")
...
m_p_traitmatch_ict <- read.csv("./Results_new/m_p_traitmatch_ict.csv") %>% column_to_rownames(var = "X")
...
# gather model data
load("./Results_land_new/Sensitivity_trait.RData")


# Calculate parasite global extinction

Ext_new <-  Sensitivity_trait %>%
...
h_abd_land <- interaction_ict_df %>% 
  dplyr::rename(patch_no = patch_id) %>% 
...
nh_df_a <- interaction_ict_df %>% 
  dplyr::rename(patch_no = patch_id) %>% 
  mutate(patch_no = as.integer(patch_no)) %>% 
  inner_join(land_ict) %>%
  select(forest_cover,p_value, Land_use, patch_no, parasite_species) %>% 
  group_by(forest_cover,p_value, Land_use, patch_no, parasite_species) %>%
...
new_net_mean_ict$PCA_axis1 <- pca_result$x[, 1] #connectance positive, size and modularity negative


outcomes_ict <- nh_df_a %>%
  select(- Mean_count) %>%
  inner_join(h_abd_land) %>%
  mutate_if(is.character, as.factor) %>% 
  inner_join(m_p_traitmatch_ict) %>% 
  select(-c(Count)) %>% 
  inner_join(Ext_new) %>% 
  inner_join(new_net_mean_ict)

# Analyse outcomes distribution

outcomes_ict$Mean_rel_sc <- as.numeric(scale(outcomes_ict$Mean_rel))
outcomes_ict$Amatch_sc <- as.numeric(scale(outcomes_ict$A_match))
outcomes_ict$Host_abd_sc <- as.numeric(scale(outcomes_ict$Host_abd))
outcomes_ict$Ext_rate_sc <- as.numeric(scale(outcomes_ict$Ext_rate))

hist(outcomes_ict$Mean_rel_sc)
hist(outcomes_ict$Amatch_sc)
hist(outcomes_ict$Host_abd_sc)
hist(outcomes_ict$Ext_rate_sc)

# save data
write.csv(outcomes_ict, "./Results_land_new/outcomes_ict.csv")
