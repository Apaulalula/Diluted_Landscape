...
# gather model data
Sensitivity_ic <- data.frame(forest_cover=double(),
                                p_value=double(),
                                patch_no=integer(),
                                guild= vector(),
                                species = vector(),
...
for(f in c(0.1,0.3,0.5,0.7)){
  for(p in c(0.01,0.25,0.5)){
    ...
    l <- read.csv(paste0("./Output_v2b_new/IC_metanetwork_fc",f,"_p",p,"_er1_ec0.5_cr1_cc0.6_mr0.5_mc0.7_phi0.5_alpha0.2_eps5.csv"), quote="\"", comment.char="")
    ...
    Sensitivity_ic <- rbind(Sensitivity_ic, l)
  }
}

glimpse(Sensitivity_ic)

save(Sensitivity_ic,file="./Results_new/Sensitivity_ic.RData")
...
for(p in c(0.01,0.25,0.5)){
  for(f in c(0.1,0.3,0.5,0.7)){
    ...
    var_name <- paste0("IC_p", p, "_lnet_", f)
...
for(p in c(0.01,0.25,0.5)){
  for(f in c(0.1,0.3,0.5,0.7)){
    ...
    filtered_matrices <- lapply(get(paste0("IC_p", p,"_lnet_", f)), function(mat) {
...
# Loop through combinations of alpha, p_value, and forest_cover
  for(f in c(0.1, 0.3, 0.5, 0.7)) {
    for(p in c(0.01, 0.25, 0.5)) {
      ...
      local_net <- get(paste0("IC_p", p, "_lnet_", f))
...
interaction_ic_df <- dplyr::bind_rows(interaction_list)
str(interaction_ic_df)

# Save data frames

write.csv(interaction_ic_df, "./Results_land_new/interaction_ic_df.csv")
