...
for(p in c(0.01,0.25,0.5)){
  for(f in c(0.1,0.3,0.5,0.7)){ 
    ...
    result_hosts <-  Sensitivity_ic %>% 
      filter(p_value == p) %>% 
      filter(forest_cover == f) %>%  
      filter(guild== "resources")
    
    result_parasites <-Sensitivity_ic %>% 
      filter(p_value == p) %>% 
      filter(forest_cover == f) %>% 
    ...
    var_name <- paste0("IC_p", p,"TraitMatch_", f)
...
trait_match_ic_mean = data.frame(NULL)

for(p in c(0.01,0.25,0.5)){
  for(f in c(0.1,0.3,0.5,0.7)){
    
    TraitMatch <- get(paste0("IC_p", p,"TraitMatch_", f))[["matching_species"]] %>%
      filter(match != is.na(match)) %>% 
      dplyr::mutate(p_value = p,
                    forest_cover = f) %>% 
      inner_join(land_pv) %>% 
      dplyr::group_by(forest_cover, p_value, Land_use, species, guild) %>% 
      dplyr::summarise(A_match = mean(match),SD_match= sd(match), A_mtheta = mean(match_theta), A_nopartners= mean(no_partners), .groups = "drop")
    
    trait_match_ic_mean = rbind(trait_match_ic_mean, TraitMatch)
    }
  }
...
write.csv(trait_match_ic_mean, "./Results_land_new/trait_match_ic_mean.csv")
...
m_p_traitmatch_ic <- trait_match_ic_mean %>% 
...
# save data
write.csv(m_p_traitmatch_ic, "./Results_land_new/m_p_traitmatch_ic.csv")
