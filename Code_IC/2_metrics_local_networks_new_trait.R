...
for(f in c(0.1,0.3,0.5,0.7)){
  for(p in c(0.01,0.25,0.5)){
    ...
    result_matrices <- get(paste0("ICT_p", p, "_lnet_", f))
    ...
    Net_metric_ict <- rbind(Net_metric_ict, groups.metric)
    }
  }


Net_metric_ict1 <- Net_metric_ict %>% 
  separate(Local, c(NA, "patch_no")) %>% 
  mutate(patch_no = as.integer(patch_no)) %>% 
  inner_join(land_ict) %>% 
  select(patch_no, forest_cover, p_value, Land_use, Size, NODF, Modularity, Connectance) %>% 
  dplyr::rename(patch_id = patch_no) %>% 
  glimpse()


############### Summarise metrics ###########################
new_net_mean_ict <- Net_metric_ict %>% 
  separate(Local, c(NA, "patch_no")) %>% 
  mutate(patch_no = as.integer(patch_no)) %>% 
  inner_join(land_ict) %>% 
  select(patch_no, forest_cover, p_value, Land_use, Size, NODF, Modularity, Connectance) %>% 
...
write.csv(new_net_mean_ict, "./Results_land_new/new_net_mean_ict.csv")
