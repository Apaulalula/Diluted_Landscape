...
for(f in c(0.1,0.3,0.5,0.7)){
  for(p in c(0.01,0.25,0.5)){
    ...
    result_matrices <- get(paste0("IC_p", p, "_lnet_", f))
    ...
    Net_metric_ic <- rbind(Net_metric_ic, groups.metric)
    }
  }


Net_metric_ic1 <- Net_metric_ic %>% 
...
write.csv(new_net_mean_ic, "./Results_land_new/new_net_mean_ic.csv")
