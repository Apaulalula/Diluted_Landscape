...
for(p in c(0.01,0.25,0.5)){
  for(f in c(0.1,0.3,0.5,0.7)){ 
    ...
    result_hosts <-  Sensitivity_trait %>% 
      filter(p_value == p) %>% 
      filter(forest_cover == f) %>%  
      filter(guild== "resources")
    
    result_parasites <-Sensitivity_trait %>% 
      filter(p_value == p) %>% 
      filter(forest_cover == f) %>% 
    ...
    var_name <- paste0("ICT_p", p,"TraitMatch_", f)
...
trait_match_ict_mean = data.frame(NULL)

for(p in c(0.01,0.25,0.5)){
  for(f in c(0.1,0.3,0.5,0.7)){
    
    TraitMatch <- get(paste0("ICT_p", p,"TraitMatch_", f))[["matching_species"]] %>%
    ...
    trait_match_ict_mean = rbind(trait_match_ict_mean, TraitMatch)
  }
}
...
str(trait_match_ict_mean)

m_p_traitmatch_ict <- trait_match_ict_mean %>% 
...
# save data
write.csv(m_p_traitmatch_ict, "./Results_land_new/m_p_traitmatch_ict.csv")
