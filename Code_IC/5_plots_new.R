...
######## Load Data ######## 
outcomes_ic <- read.csv("./Results_land_new/outcomes_ic.csv") %>% column_to_rownames(var = "X")

# Work on data set ################################### 

# Verify unique levels of categorical variables
outcomes_ic$parasite_species <- factor(outcomes_ic$parasite_species)
outcomes_ic$Landscape <- factor(outcomes_ic$Landscape)
...
Ex_plot_data <- outcomes_ic %>%  
  group_by(forest_cover, p_value) %>%
  # mutate(IC = case_when(IC == "75%" ~ "75% initial occupancy",
  #                           IC == "100%" ~ "100% initial occupancy"
  #                           )) %>% 
  dplyr::summarise(
...
Ex_plot <- ggplot(Ex_plot_data, aes(x= as.factor(forest_cover), y= mean_value)) + 
  ...
  labs(title = "75% initial occupancy")+
  #facet_wrap(~IC)+
...
png(filename="./Results_new/ext_md_ic.png", width=10, height=8, units="cm", res=600)
plot(Ex_plot)
dev.off()
...
png(filename="./Results_new/PCA_ns_ic.png", width=12, height=20, units="cm", res=600)
plot(final_plot)
dev.off()
...
png(filename="./Results_new/DG_plot_ic.png", width=15, height=7, units="cm", res=600)
plot(dg_plot1)
dev.off()
...
png(filename="./Results_new/P_rd_ic.png", width=12, height=20, units="cm", res=600)
plot(final_plot)
dev.off()

## Trait matching plots ####
### Plot relation between trait matching, forest cover and network structure ####
TM_plot1 <-outcomes_ic %>% 
  select(-p_value) %>% 
  # mutate(IC = case_when(IC == "75%" ~ "75% initial occupancy",
  #                         IC == "100%" ~ "100% initial occupancy"
  # )) %>% 
...
png(filename="./Results_new/P_tm_ic.png", width=12, height=20, units="cm", res=600)
plot(final_plot)
dev.off()
