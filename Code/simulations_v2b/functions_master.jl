...
function update_patches_sequentially(x_state, x_r, x_c, z_r, z_c, x_r_old, x_c_old, z_r_old, z_c_old, n_r, n_c, pf_r, e_r, e_c, c_r, c_c, theta_r, theta_c, alpha, m_r, m_c, epsilon, phi, n, M_inc)
  ...
  for i = 1:n
    for j = 1:n
      ...
      z_r, z_c, x_c = simulate_coevolution_or_evolution(z_r, z_c, x_c, r_current_sp, c_current_sp, theta_r, theta_c, alpha, m_r, m_c, epsilon, phi, M_inc, i, j)
...
function dynamics(network, forestCover, pvalue, tmax, e_r, e_c, c_r, c_c, phi, alpha, m_r, m_c, epsilon)
